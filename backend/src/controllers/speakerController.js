...
export const getSpeakers = async (req, res, next) => {
  try {
    const speakers = await speakerService.getSpeakers(req.user.organization);
    ok(res, speakers);
  } catch (e) {
    next(e);
  }
...
export const updateSpeaker = async (req, res, next) => {
  try {
    ...
    const speaker = await speakerService.updateSpeaker(req.user.organization, req.params.id, data);
...
export const deleteSpeaker = async (req, res, next) => {
  try {
    await speakerService.deleteSpeaker(req.user.organization, req.params.id);
